        ...
        <div className="mb-40">
          {orders?.map((order) => (
            ...
            >
              ...
              <StatusStyling status={order.status} onClick={() => {setOpenStatusMenu(prev => prev === order.id ? null : order.id)}} openStatusMenu={openStatusMenu === order.id} orderId={order.id}/>
...
const StatusDropDown = ({orderId}) => {
  const classes = "block text-sm/6 text-left hover:bg-backgroundDark w-full px-4 active:bg-textLight active:text-background";
  return (
    <div className="z-10 py-1 bg-background block absolute shadow-lg rounded-md">
      <button className={classes} onClick={() => {updateOrderStatus(orderId, "delivered")}}>Draft</button>
...
function StatusStyling({status, onClick, openStatusMenu, orderId}) {
  ...
  switch (status) {
    case 'draft':
      return (
        <div className="relative group">
          <div className="bg-backgroundDark/40 text-textLight group break-words py-0.5 px-2 w-fit rounded-md"
                onClick={onClick}>
            {status}
    ...
    case 'pending':
      return (
        <div className=" bg-progressYellow/40 text-yellow-600 group break-words py-0.5 px-2 w-fit rounded-md"
              onClick={onClick}>
          {status}
          <svg className="inline size-0 pl-1 group-hover:size-fit" width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M9.5 1L5 5L0.5 1" stroke="currentColor" strokeWidth="1.5"/>
    ...
    case 'shipped':
      return (
        <div className="bg-secondary/40 text-primaryDark group break-words py-0.5 px-2 w-fit rounded-md"
              onClick={onClick}>
          {status}
          <svg className="inline size-0 pl-1 group-hover:size-fit" width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M9.5 1L5 5L0.5 1" stroke="currentColor" strokeWidth="1.5"/>
    ...
    case 'delivered':
      return (
        <div className="bg-successGreen/40 text-lime-600 group break-words py-0.5 px-2 w-fit rounded-md"
              onClick={onClick}>
          {status}
          <svg className="inline size-0 pl-1 group-hover:size-fit" width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M9.5 1L5 5L0.5 1" stroke="currentColor" strokeWidth="1.5"/>
    ...
    case 'cancelled':
      return (
        <div className="bg-failRed/40 text-orange-700 group break-words py-0.5 px-2 w-fit rounded-md"
              onClick={onClick}>
          {status}
          <svg className="inline size-0 pl-1 group-hover:size-fit" width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M9.5 1L5 5L0.5 1" stroke="currentColor" strokeWidth="1.5"/>
